...
var Component = {
    Cloud: {
        init: function (isInit) {
            ...
            // clouds.push(obj);
            var object = { obj: obj, speed: CLOUD_SPEED, direction: DIRECTION_HOR, width: CLOUD_WIDTH, height: CLOUD_HEIGHT};
            objects.push(object);
        }
    },
    Bird: {
        init: function () {
            ...
            // birds.push(obj);
            var object = { obj: obj, speed: BIRD_SPEED, direction: DIRECTION_HOR, width: BIRD_WIDTH, height: BIRD_HEIGHT};
            objects.push(object);
        }
    },
    Star: {
        init: function () {
            ...
            // stars.push(obj);
            var object = { obj: obj, speed: STAR_SPEED, direction: DIRECTION_VER, width: STAR_WIDTH, height: STAR_HEIGHT};
            objects.push(object);
        }
    },
    Parachute: {
        init: function () {
            ...
            var object = { obj: obj, speed: PARACHUTE_SPEED, direction: DIRECTION_VER, width: PARACHUTE_WIDTH, height: PARACHUTE_HEIGHT};
            objects.push(object);
        }
    },
    Airplane: {
        ...
        goX: function (distance) {
            var leftCurrent = AIRPLANE.position().left;
        ...
        goY: function (distance) {
            var topCurrent = AIRPLANE.position().top;
    ...
    FuelCounter: {
        init: function () {
            ...
            function reduce() {
                ...
                setTimeout(reduce, FUEL_COUNTER_TIME_CHANGED);
            }
            ...
            function start() {
                ...
                if (isRunning) {
                    for (var i = 0; i < length; i++) {
                        ...
                        // Horizontal
                        if(object.direction === DIRECTION_HOR) {
                            // Xử lý va chạm
                            if (Helper.isCollision(objectJq) && objectJq.hasClass('elm-bird')) {
                                ...
                                var leftCurrent = objectJq.position().left;
                                if (leftCurrent <= -object.width) {
                                    remove(i);
                                } else
                                objectJq.css({left: leftCurrent - object.speed + 'px', 'animation-play-state': 'running'});
                            }
                        }
                        // Vertical
                        else {
                            ...
                            if (Helper.isCollision(objectJq)) {
                                ...
                                // // Xử lý di chuyển
                                var topCurrent = objectJq.position().top;
                                if(objectJq.hasClass('elm-parachute'))
                                    console.log(topCurrent + objectJq.attr('class') +  object.speed);
                                if (topCurrent >= GAME_AREA_HEIGHT) {
                                    remove(i);
                                } else
                                    objectJq.css({top: topCurrent + object.speed + 'px', 'animation-play-state': 'running'});
                            }
                        }
                    }
                }
